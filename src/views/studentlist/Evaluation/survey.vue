  ...
  <div class="main">
    ...
    <div class="data">
      ...
      >
        ...
        <el-table-column prop="view_name" label="量表名称"></el-table-column>
        <el-table-column prop="topic_count" label="题目数量"></el-table-column>
        <el-table-column prop="start_time" label="开始时间"></el-table-column>
        <el-table-column prop="expire_time" label="截止时间"></el-table-column>
        <el-table-column  label="完成">
            <template slot-scope="scope">
              <span  v-if="scope.row.end_test_time" style="color:green">已完成</span>
...
export default {
  ...
  methods: {
    getlist(currentpage, pagesize) {
      ...
      this.axios
        .post("/api/v1/scale/getScaleTestList", {
          page: currentpage,
          size: pagesize
        })
        .then(function(res) {
    ...
    handleEdit(index, row) {
      ...
      this.form1.scaleid = row.sbi_id;
      this.form1.name = row.name;
      this.form1.unit_name = row.unit_name;
              this.$router.push({path:'/test',query:{id:row.id,name:row.view_name,scaleid:row.sbi_id}})
