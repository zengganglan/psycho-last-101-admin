  ...
  <div class="main">
      ...
      >
        ...
        <el-table-column label="序号">
          <template slot-scope="scope">
            <span>{{1+scope.$index}}</span>
          </template>
        </el-table-column>
        <el-table-column property="name" label="学校名称"></el-table-column>
        <el-table-column property="desc" label="学校简介"></el-table-column>
        ...
        <el-table-column label="操作">
          ...
          size="mini"
          type="danger"
            @click.stop="handlepage(scope.$index, scope.row)">设置</el-button> -->
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog title="学校管理编辑" :visible.sync="dialogFormVisible1">
      <el-form :model="form1">
        ...
        <el-form-item label="学校简介" :label-width="formLabelWidth">
          <el-input v-model="form1.unit_name" autocomplete="off" style="width: 500px;"  type='textarea'></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
     ...
     <el-dialog title="学校界面管理" :visible.sync="dialogFormVisible2" width='400px'>
        <el-tree
        show-checkbox
        :data="data4"
        :props="defaultProps"
...
export default {
  ...
  methods: {
    getlist(currentpage, pagesize) {
      ...
      var that = this;
      this.axios
        .post("/api/v1/super/college/list", {
          page: currentpage,
          size: pagesize
        })
        .then(function(res) {
    ...
    editsure() {
      var that = this;
      this.axios
        .post("/api/v1/super/college/save", {
          id: this.form1.id,
    ...
    handleDelete(index, row) {
      ...
      this.$confirm("此操作将永久删除该信息, 是否继续?", "提示", {
        ...
        .then(() => {
          //  点击确定开始删除
          this.axios
            .post("/api/v1/super/college/delete", { id: id })
            .then(function(res) {
              if (res["data"]["code"] == 0) {
                that.$message("删除成功");
                that.getlist(that.page.currentpage, that.page.pagesize);
