  ...
  <div class="main">
    ...
    <ul>
      <li class="data1" v-if="tabindex==1">
        <div class="luru">修改校长资料</div>
        <table class="a1">
          <tr>
            <td>所属学校：</td>
            <td>
              <!-- <span>{{detail.id}}</span> -->
              <!-- <el-select v-model='schoolid' style="width:202px">
                <el-option v-for="(item,index) in select" :key="index" :lable='item.name' :value='item.id'></el-option>
              </el-select>-->
              <el-select v-model="detail.college_id" :disabled="disabledInput
...
export default {
  ...
  methods: {
    ...
    baseinfo(id) {
      ...
      var that = this;
      that.axios
        .get("/api/v1/super/user/detail/"+id)
        .then(function(res) {
          var detail = res["data"]["data"];
         
          // 修改和增加要筛选掉已有超管的学校 但是修改同时要把本身的学校显示在界面上
           var obj={id:detail.college_id,name:that.name}
           that.select.push(obj)
          var arr = [];
          //院校
          var data=that.select
           data.map(function(item, index){
            if (item.id == detail.college_id) {
              detail['college']=item.name
            }
          });
          // 男女
          if (detail.sex == 0) {
    ...
    setadmin() {
      this.values.password=this.detail.password;
      this.values.account = this.detail.account;
      ...
      this.values.college_id = this.detail.college_id;
      var that = this;
      console.log(this.values,that.detaildata);
      this.values.sex = this.values.sex + "";//判断需要把数字变成字符串 避免是0为false
      // 修改的话没有字段为空就不要传表示用户不改变。判断有没有空值 
       var obj={}
      ...
      console.log(obj, this.values,data)
      obj['id']=this.$route.query.id     
      that.axios
        .post("/api/v1/super/user/save", obj)
        .then(function(res) {
          if (res["data"]["code"] == 0) {
            that.$message("编辑用户成功");
            that.$router.go(-1)
