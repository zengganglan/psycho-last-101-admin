...
export default {
  ...
  watch: {
    "$route.path": function() {
      //左侧栏存在的地方都会检测 首页从别处点击到菜单栏某个路由菜单栏没有对应的变化
      this.navbardata1();
    }
  },
  beforeMount() {
  ...
  methods: {
    ...
    handleOpen(key, keyPath, event) {
      console.log(key, keyPath, this.$route.path,event);
    },
    ...
    navbardata1() {
      ...
      this.axios
        ...
        .then(function(res) {
          ...
          var obj=[]
          // 1根据字段flag把按钮权限过滤掉。把data变成一维数组 把权限按钮筛选出来放到数组
          var data2 = that.NestedToFlat(data);
          data2.map((item, index) => {
            if (item.flag == 0) {
              that.permission.push(item);
            } 
          });
        ...
        //  console.log(JSON.parse(sessionStorage.getItem('buttonspermissions')))
          ...
          a(data,obj)
          // console.log(that.permission, a(data,obj));
          // 3根据sort把数据进行排序处理  数组不会影响顺序
          var arr = [];
          for (var key in data) {
          ...
          that.navbardata.map((item, index) => {
            ...
            if (
              that.$route.path.indexOf("/" + item["menu_route"] + "/") !== -1
            ) {
              console.log(item["menu_route"], index);
              index1 = index;
            }
            if (that.$route.path == "/" + item["menu_route"]) {
              // 如果第一级路径相同就覆盖因为前面条件也成立这个要覆盖前面的。
              index1 = index;
            }
          });
