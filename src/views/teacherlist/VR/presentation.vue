  ...
  <div class="main">
    ...
    <div class="data vrdata">
      ...
      >
        ...
        <el-table-column property="job_num" label="学号"></el-table-column>
        <el-table-column property="name" label="学生姓名"></el-table-column>
        <el-table-column property="unit_group_name" label="所属组别"></el-table-column>
        <el-table-column property="create_time" sortable label="最近上传"></el-table-column>
    ...
    <el-dialog title="学号添加" :visible.sync="dialogFormVisible">
      <el-form :model="form">
        <el-form-item label="学号" :label-width="formLabelWidth">
          <el-input v-model="form.id" autocomplete="off" style="width: 210px;"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  data() {
    return {
      ...
      // 头部组件信息
      headson: ["VR游戏数据",'点击个用户数据查询某个用户的所有VR报告'],
      tableData: [],
      multipleSelection: [],
      // 页码
      page: {
  ...
  methods: {
    getlist(currentpage, pagesize) {
      ...
      var that = this;
      this.axios
        .post("/api/v1/admin/vr/group/list", {
          page: currentpage,
          size: pagesize
        })
        .then(function(res) {
    ...
    handleDelete(index, row) {
      ...
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          //  点击确定开始删除
          this.axios
            .post("/api/v1/admin/vr/group/delete", { id: id })
            .then(function(res) {
              console.log(res);
              if (res["data"]["code"] == 0) {
                that.$message("删除成功");
