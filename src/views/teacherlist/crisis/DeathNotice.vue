  ...
  <div class="main">
    ...
    <div>
      ...
      <table class="tab">
        ...
        <tbody>
          <tr>
            <td v-for="(item,index) in arrs" :key="index">
              <div >
                <span v-for="(item1,index1) in item['obj']" :key='index1' @click="funtitle(item1['id'],$event)" ref="nav"  >{{item1['title']}}</span>
                <!-- <span>学生自杀时</span>
                <span>死因不明时</span>
          ...
          <el-form-item label="内容" :label-width="formLabelWidth">
            <div id="editorElem" style="text-align:left">
              ...
              <p>
                ...
                <div v-else-if="titleflag==2">
                   <p>我们非常伤心地告诉大家，我们的一个老师_________。我们希望大家知道，我们将尽一切努力帮助大家。</p>
              <p>
                有关自杀方面的很多问题我们不能马上回答。在此期间可能会出现一些谣言，我们请大家不要传播这些谣言。如果我们知道事情的进展，会在第一时间让大家知道确切的消息。
                自杀是一个非常复杂的问题。通常有精神疾病，如抑郁症导致。抑郁症是一个人不能清醒的看待问题，解决问题。我们有时不能准确识别或注意到这种疾病，有些时候，有这种疾病的人可能会有明显的症状或体征，但有一点是可以肯定的，如果治疗是不会自杀的。
                每一个人对_______的去世会有自己的反应，我们要相互尊重，感到悲伤是正常的反应。你们当中有些人可能不十分了解_________，没有受到影响，而其他人可能会经历巨大悲痛。有些人可能会感到不能集中注意力学习，有些人可能感觉集中注意力学习是很好的缓解情绪的方式。
              ...
              <p>
               ...
               <div v-else>
                   <p>我们非常伤心地告诉大家,我们希望大家知道，我们将尽一切努力帮助大家。</p>
              <p>
...
export default {
  ...
  methods: {
    ...
    funtitle(id,event){
        this.$refs.nav.map((item,index)=>{
         item.style.color=''
        })
         event.target.style.color='#009dd9';
        console.log(this.$refs.nav,event.target)
       if (id==1) {
         this.titleflag=1
       }else if (id==2) {
         this.titleflag=2
