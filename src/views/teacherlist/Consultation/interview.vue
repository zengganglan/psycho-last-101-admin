<template>
  <div class="main">
    ...
    <el-dialog title="新增团辅访谈安排" :visible.sync="dialogFormVisible">
      <el-form :model="form">
  ...
  <el-date-picker
        ...
        <el-form-item label="时间止" :label-width="formLabelWidth">
          <el-date-picker
           style="width:230px"
            ...
            type="datetime"
            placeholder="截止日期大于开始日期"
            value-format="yyyy-MM-dd HH:mm:ss"
            :picker-options="pickerOptions3"
          ></el-date-picker>
        </el-form-item>
        <el-form-item label="位置" :label-width="formLabelWidth">
          <el-input v-model="form.location" autocomplete="off" style="width: 230px;" type="text"></el-input>
        </el-form-item>
        <el-form-item label="访谈对象" :label-width="formLabelWidth">
          ...
          <!-- <el-select v-model="form.name" placeholder="类型是团辅请选择组别"  value-key v-if="typeflag">
            <el-option
           ...
           <el-input v-model="form.user_id" autocomplete="off" style="width: 230px;" type="number" min='0' placeholder="类型是个人请输入学号" v-else></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="访谈时间编辑" :visible.sync="dialogFormVisible1">
      <el-form :model="form1">
...
export default {
  data() {
    return {
        types:[{name:'团辅',id:1},{name:'个人',id:2}],
        typeflag:true,
       pickerOptions1: {
          disabledDate(time) {
  ...
  methods: {
    ...
    makesure() {
      ...
      this.axios
        .post("/api/v1/admin/coach/add",obj)
        .then(function(res) {
          console.log(res);
          if (res["data"]["code"] == 0) {
            that.$message("增加成功");
    ...
    beginDate(){
      ...
      return {
         disabledDate(time) {
              if(that.form.send_time){
              return new Date(that.form.send_time).getTime() > time.getTime() 
            } else if (that.form.end_time) {
              return new Date(that.form.end_time).getTime() < time.getTime() 
            } else {
     ...
     endDate(){
      ...
      return {
         disabledDate(time) {
              ...
              return new Date(that.form.start_time).getTime() > time.getTime() 
            }else{
              return time.getTime() < Date.now()-24*60*60*1000//开始时间不选时，结束时间最大值小于等于当天       
            }
          },
