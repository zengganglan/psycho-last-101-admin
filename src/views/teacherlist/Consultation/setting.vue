  ...
  <div class="main">
    ...
    <div class="data">
      ...
      >
        ...
        <el-table-column prop="name" label="功能室名称"></el-table-column>
        <el-table-column prop="desc" label="功能室简介"></el-table-column>
        <!-- <el-table-column prop="college_name" label="所属学校"></el-table-column> -->
        <el-table-column label="操作" width="200">
          <template slot-scope="scope">
            <el-button size="mini" @click.stop="handleEdit(scope.$index, scope.row)">修改</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog title="功能室添加" :visible.sync="dialogFormVisible">
      <el-form :model="form">
        ...
        <el-form-item label="功能室简介" :label-width="formLabelWidth">
          <el-input
    ...
    <el-dialog title="功能室编辑" :visible.sync="dialogFormVisible1">
      <el-form :model="form1">
        <el-form-item label="功能室名称" :label-width="formLabelWidth">
...
export default {
  ...
  methods: {
    ...
    getlist(currentpage, pagesize) {
      ...
      that.axios.get("/api/v1/admin/tool/select/college").then(res => {
        if (res["data"]["code"] == 0) {
          that.select = res["data"]["data"];
           data.map((item,index)=>{
                that.select.find((item1,index1)=>{
                  if (item.college_id==item1.id) {
                     return  item.college_name=item1.name                      
                  }else{
                    item.college_name=null
                  }
                  // /map中item.college_name=null如果加上else会覆盖点原来进入条件为真的循环
                })
            })
            that.tableData =data
            console.log(data)
    ...
    editsure() {
      var that = this
      this.axios
        .post("/api/v1/admin/func/save", {
          id: this.form1.id,
          name: this.form1.name,
          desc: this.form1.unit_name
        })
        .then(function(res) {
          console.log(res);
          if (res["data"]["code"] == 0) {
    ...
    handleDelete(index, row) {
      ...
      var that = this;
      this.axios
        .post("/api/v1/admin/func/delete", { id: id })
        .then(function(res) {
          console.log(res);
          if (res["data"]["code"] == 0) {
            that.$message("删除成功");
