<template>
    ...
    <ul>
      <li class="data1" v-if="tabindex==1">
        ...
        <ul class="a1">
          ...
          <li>
            <span>起始学号：</span>
            <input type="number" placeholder="请输入学号" v-model="startId"  min="0">
          </li>
          <li>
            <span>导入数量：</span>
             <!-- <el-input type="age" v-model.number="number" autocomplete="off"></el-input> -->
            <input type="number" placeholder="请输入数量 不超过1000" v-model="number" min="0" onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}" >
          </li>
          <li>
            <span>初始密码：</span>
            <input
          ...
          <li ><el-button type="primary" plain @click="newStudent">提交</el-button></li>
        </ul>
      </li>
      <li class="data2" v-else-if="tabindex==2">
        ...
        <ul class="a2">
          ...
          <li class="submit2" >
            <el-button type="primary" :disabled='disabled'  @click="submit2">上传</el-button>
  ...
  <ol style="font-size: 12px;">
    ...
    <li>工作表列按照以下顺序排列且至少包含以下列：学号/工号	 密码	姓名	性别	民族	出生日期	电话	身份证
（共8列），其中：
      <ul>
        <li>学号：英文（大小写不限）、数字或者组合，不能出现汉字等特殊字符，可留空；</li>
    ...
    <li>每次导入会检测是否已存在，已存在导入失败。若通过EXCEL导入账号，则手机号码无需验证。
EXCEL批量导入时，学号=账号。</li>
  </ol>
</fieldset>
      </li>
  ...
  methods: {
    ...
    upload() {
      ...
      if (filePath.indexOf("xlsx") != -1 || filePath.indexOf("xls") != -1) {
        ...
        // console.log(this.getObjectURL(file),event.target.value)
        var name = file.name;
        this.filename = name;
        this.disabled=false
    ...
    newStudent() {
      ...
      if (this.number >= 1000) {
        this.number = 1000;
        this.$message('导入数量超过1000默认1000处理')
