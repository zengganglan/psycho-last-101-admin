      ...
      <li class="data1" v-if="tabindex==1">
        ...
        <table class="a1">
          ...
          <tr class>
            ...
            <td>
              ...
              <template >
                  ...
                  <input type="button" name="sex1" value="男" ref="sex1" @click="checkSex($event)" :style="{'backgroundColor':values.sex==1? '#5bb95a':''}" v-bind:disabled="disabledInput">
                  <input name="sex2" type="button" value="女" @click="checkSex($event)" ref="sex2" :style="{'backgroundColor':values.sex==0? '#5bb95a':''}" v-bind:disabled="disabledInput">
                           <span>*</span>
          ...
          <tr class='rolegroup'>
            ...
            <td>
              <template >
                <el-select v-model="detail.auth_group_id" placeholder="请选择"  v-bind:disabled="disabledInput">
                <el-option
   ...
   changedisable(){
    ...
    tab(index) {
     ...
     baseinfo(id){
        ...
        console.log(id)
        var that = this 
        that.axios.post("/api/v1/admin/user/detail",{uid:id}).then(function(res){
          console.log(res)
    ...
    setadmin() {
      ...
      this.values.sex;
      this.values.birth_date=this.detail.birth_date
     ...
     this.values.sex = this.values.sex + "";//判断需要把数字变成字符串 避免是0为false
      ...
      console.log(this.values)
       ...
       if (JSON.stringify(obj) == "{}") {
          that.$message("没有修改信息请返回");
        return 
      }
      that.axios.post('api/v1/admin/user/editUser?id='+that.id,obj).then(function(res){
        console.log(res)
         if (res["data"]["code"] == 0) {
            that.$message("编辑校长用户成功");
            that.$router.go(-1)
