...
;
  ...
  methods: {
    ...
    getList() {
      var that =this
       this.axios.get("/api/v1/admin/user/roleGroup").then(function(res){
         if (res['data']['code']==0) {
   ...
   // 分页·
    ...
    complete() {
      this.addflag = false;
      var that = this
        this.axios.post("/api/v1/admin/user/addRoleGroup",{
    ...
    queding(id,item) {
      ...
      var that = this
      this.axios.post("/api/v1/admin/user/editRoleGroup",{
        id:ids,
    ...
    del(id) {
      var that  = this
       ...
       this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          //  点击确定开始删除
            this.axios.get('/api/v1/admin/user/deleteRoleGroup?id='+id).then(function(res){
                 if (res['data']['code']==0) {
                    that.$message('删除成功')
                    that.getList()
                 }else{
    ...
    colorcancel() {
     ...
     getCheckedNodes() {
       var data=this.$refs.tree.getCheckedNodes().concat(this.$refs.tree.getHalfCheckedNodes())
        var ids= []
        data.map(item=>{
            ids.push(item.id)
        })
        if (ids.length!==0) {
           var that= this
          that.axios.post("/api/v1/admin/user/editRoleGroup",{id:that.ichecks,rules:ids}).then(function(res){
        if (res["data"]["code"]==0) {
           that.$message("权限设置成功")
