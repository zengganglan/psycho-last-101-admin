...
<template>
  <div class="content">
    ...
    <div align="center">
      <b>量表：同学关系问卷</b> 【
      <a href="#" target="" @click="exportss" style="color:blue">测试报告</a>】
    </div>
    <fieldset>
      <legend>测试结果</legend>
      <table class="tb_stat">
        <tbody>
          <tr>
            <th>因子</th>
              <td  v-for="(item,key,index) in facname"
        :key="index">{{item}}</td>
          </tr>
          <tr>
            <td>得分</td>
            <td  v-for="(item,key,index) in score"
        :key="index">{{item}}</td>
    ...
    <fieldset>
      <legend>量表剖析图 【<span @click="print(1)">饼图  </span><span @click="print(2)">折线图  </span><span @click="print(3)">柱形图</span>】</legend>
      <div class="chart" ref="chart"></div>
    </fieldset>
    <fieldset style="line-height:150%;">
      <legend>结果解释</legend>
      <div  v-for="(item,key,index) in factors"
        :key="index" >
        <span v-if="item['desc']"> 【{{item['desc']['factor']['name']}}】{{item['desc']['calc']['explain']}}。</span>
    ...
    <fieldset style="line-height:150%;">
      <legend>原始选择</legend>
             ...
             <el-row>
        <el-col :xs="2" :sm="2" :md="2" :lg="2"  v-for="(item,index) in answers" :key='index'><div class="grid-content bg-purple" > 
           {{index+1}}．[
           <span class="answer" v-if='item.length>=0'>
                <i  v-for="(item1,index1) in item" :key='index1'>
                  {{item1}},
                </i>
              </span>
              <span class="answer" v-else>{{item}}</span>]</div></el-col>
      ...
      this.axios
        ...
        .then(function(res) {
          if (res["data"]["code"] == 0) {
            ...
            console.log(res['data']['data']['result'],that.factors, that.answers)
            var facname=[]
            var score=[]
